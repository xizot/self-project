...
import { AutomationTask } from '@/src/lib/types';
...
import { Calendar, Clock, Edit, Play, Search, Zap } from 'lucide-react';
...
export default function CronManagement() {
  ...
  const getScheduleColor = (schedule: string) => {
    ...
    if (schedule.includes('m'))
      return 'bg-green-100 text-green-800 border-green-300';
    if (schedule.includes('h'))
      return 'bg-yellow-100 text-yellow-800 border-yellow-300';
    if (schedule.includes('d'))
      return 'bg-purple-100 text-purple-800 border-purple-300';
    if (schedule.includes('w'))
                            ...
                            <TableCell>
                              {task.next_run_at ? (
                                <div>
                                  ...
                                  <div
                                    className={`text-xs ${nextRunStatus.color}`}
                                  >
                                    {nextRunStatus.text}
                                  </div>
                                </div>
