...
try {
  ...
  const hasWebhookId = automationTasksInfo.some(
    (col) => col.name === 'webhook_id'
  );
  ...
  if (!hasWebhookId) {
    ...
    db.exec(
      'CREATE INDEX IF NOT EXISTS idx_automation_tasks_webhook ON automation_tasks(webhook_id)'
    );
...
try {
  ...
  if (!hasType) {
    db.exec(
      `ALTER TABLE passwords ADD COLUMN type TEXT NOT NULL DEFAULT 'password';`
    );
...
try {
  const defaultScripts = [
    {
      ...
      description:
        'Script tự động lấy giá vàng từ trang PNJ (https://www.pnj.com.vn/site/gia-vang)',
      path: 'scripts/get-gold-price.js',
    },
    {
      name: 'Lấy Jira Tasks',
      description:
        'Script tự động lấy các task được assign cho bạn trên Jira. Cần cấu hình credentials (Jira API token) trong Quản lý Mật khẩu.',
  ...
  for (const user of users) {
    for (const script of defaultScripts) {
      ...
      const existing = db
        .prepare(
          'SELECT id FROM automation_scripts WHERE user_id = ? AND path = ?'
        )
  ...
  if (users.length > 0) {
    console.log(
      `Seeded ${defaultScripts.length} default automation scripts for ${users.length} user(s)`
    );
  }
} catch (error: any) {
...
// Migration: Add custom schedule & body metrics to hydration_settings
try {
  const hydrCols = (
    db.prepare('PRAGMA table_info(hydration_settings)').all() as Array<{
      name: string;
    }>
  ).map((c) => c.name);

  if (!hydrCols.includes('custom_schedule')) {
    db.exec('ALTER TABLE hydration_settings ADD COLUMN custom_schedule TEXT');
  }
  if (!hydrCols.includes('weight')) {
    db.exec('ALTER TABLE hydration_settings ADD COLUMN weight REAL');
  }
  if (!hydrCols.includes('activity_level')) {
    db.exec('ALTER TABLE hydration_settings ADD COLUMN activity_level TEXT');
  }
} catch (error: any) {
  if (!error?.message?.includes('duplicate column')) {
    console.log('Hydration settings migration error:', error);
  }
}
